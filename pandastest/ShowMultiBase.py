...
class ShowMultiBase(DirectObject.DirectObject):
    ...
    def __init__(self, fStartDirect=True, windowType=None):
        ...
        if self.__dev__:
            ShowMultiBase.notify.debug('__dev__ == %s' % self.__dev__)
        else:
            ShowMultiBase.notify.info('__dev__ == %s' % self.__dev__)

        self.createBaseAudioManagers()

        if self.__dev__ and self.config.GetBool('track-gui-items', False):
            # dict of guiId to gui item, for tracking down leaks
            if not hasattr(ShowMultiBase, 'guiItems'):
                ShowMultiBase.guiItems = {}
